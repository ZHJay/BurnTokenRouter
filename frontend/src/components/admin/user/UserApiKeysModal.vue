<template>
  <BaseDialog :show="show" :title="t('admin.users.userApiKeys')" width="wide" @close="handleClose">
    <div v-if="user" class="space-y-4">
      <div class="card-inset flex items-center gap-3 rounded-xl p-4">
      ...
      <div v-else-if="apiKeys.length === 0" class="empty-state py-8"><p class="empty-state-description">{{ t('admin.users.noApiKeys') }}</p></div>
      <div v-else ref="scrollContainerRef" class="max-h-96 space-y-3 overflow-y-auto" @scroll="closeGroupSelector">
        <div v-for="key in apiKeys" :key="key.id" class="card-inset rounded-xl p-4">
          <div class="flex items-start justify-between">
            <div class="min-w-0 flex-1">
              <div class="mb-1 flex items-center gap-2"><span class="font-medium text-gray-900 dark:text-white">{{ key.name }}</span><span :class="['badge text-xs', key.status === 'active' ? 'badge-success' : key.status === 'quota_exhausted' ? 'badge-warning' : 'badge-gray']">{{ key.status }}</span></div>
          ...
          <div class="mt-3 flex flex-wrap gap-4 text-xs text-gray-500">
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                class="-mx-1 -my-0.5 flex cursor-pointer items-center gap-1 rounded-md px-1 py-0.5 transition-[background-color,transform] duration-fast ease-apple-out hover:bg-gray-100 focus-visible:outline-none focus-visible:ring-[3.5px] focus-visible:ring-[var(--accent-tint-strong)] active:scale-[0.96] dark:hover:bg-dark-700"
            ...
            <div class="flex items-center gap-1"><span class="tabular">{{ t('admin.users.columns.created') }}: {{ formatDateTime(key.created_at) }}</span></div>
          </div>
        </div>
      </div>
    </div>
  ...
  <Teleport to="body">
    <div
      ...
      ref="dropdownRef"
      class="glass-thin glass-edge animate-scale-in fixed z-[100000020] w-64 origin-top overflow-hidden rounded-xl"
    ...
    >
      <div class="max-h-64 overflow-y-auto p-1.5">
        ...
        <button
          ...
          :class="[
            'flex w-full items-center rounded-lg px-3 py-2 text-sm transition-[background-color,transform] duration-fast ease-apple-out active:scale-[0.98]',
        ...
        <button
          ...
          :class="[
            'flex w-full items-center justify-between rounded-lg px-3 py-2 text-sm transition-[background-color,transform] duration-fast ease-apple-out active:scale-[0.98]',
