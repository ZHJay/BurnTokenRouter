<template>
  <BaseDialog :show="show" :title="t('admin.users.groupConfig')" width="wide" @close="$emit('close')">
    <div v-if="user" class="space-y-6">
      <!-- 用户信息头部 -->
      <div class="flex items-center gap-4 rounded-xl bg-[var(--accent-tint)] p-5 shadow-[inset_0_0_0_0.5px_var(--hairline)]">
        <div class="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-[var(--surface)] shadow-elev-1">
          <span class="text-2xl font-semibold text-primary-600 dark:text-primary-400">{{ user.email.charAt(0).toUpperCase() }}</span>
        </div>
        <div class="flex-1">
          <p class="text-lg font-semibold tracking-[-0.014em] text-gray-900 dark:text-white">{{ user.email }}</p>
      ...
      <div v-else class="space-y-6">
        ...
        <div v-if="exclusiveGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            ...
            <span class="tabular text-xs text-gray-400">({{ exclusiveGroupConfigs.filter(c => c.isSelected).length }}/{{ exclusiveGroupConfigs.length }})</span>
          </div>
          <div class="grid gap-3">
            <div
              v-for="config in exclusiveGroupConfigs"
              :key="config.groupId"
              class="card group relative overflow-hidden p-4 transition-[background-color,box-shadow] duration-fast ease-apple-out"
            ...
            >
              <div class="flex items-center gap-4">
                ...
                <div class="flex-shrink-0">
                  <label class="relative flex h-6 w-6 cursor-pointer items-center justify-center transition-transform duration-instant ease-apple-out active:scale-[0.96]">
                    <input
                      type="checkbox"
                      ...
                      class="peer sr-only"
                    />
                    <div class="h-5 w-5 rounded-md border-2 border-gray-300 transition-all peer-checked:border-primary-500 peer-checked:bg-primary-500 peer-focus-visible:ring-[3.5px] peer-focus-visible:ring-[color:var(--accent-tint-strong)] dark:border-dark-500 peer-checked:dark:border-primary-500">
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <div class="mt-1.5 flex items-center gap-3 text-sm">
                    ...
                    <span class="text-gray-500 dark:text-gray-400">
                      {{ t('admin.users.defaultRate') }}: <span class="tabular font-semibold text-gray-700 dark:text-gray-300">{{ config.defaultRate }}x</span>
                    </span>
                  </div>
                </div>
                ...
                <div class="flex flex-shrink-0 items-center gap-3">
                  ...
                  <input
                    ...
                    class="input hide-spinner tabular w-24 font-medium"
                  />
                </div>
              </div>
            </div>
        ...
        <div v-if="publicGroups.length > 0">
          <div class="mb-3 flex items-center gap-2">
            ...
            <span class="tabular text-xs text-gray-400">({{ publicGroupConfigs.length }})</span>
          </div>
          <div class="grid gap-3">
            <div
              v-for="config in publicGroupConfigs"
              :key="config.groupId"
              class="relative overflow-hidden rounded-xl bg-green-50/50 p-4 shadow-[inset_0_0_0_0.5px_var(--hairline)] dark:bg-green-900/10"
            >
              <div class="flex items-center gap-4">
                <!-- 复选框（禁用状态） -->
                <div class="flex-shrink-0">
        ...
        <div v-if="groups.length === 0" class="empty-state">
          ...
          <p class="empty-state-description">{{ t('common.noGroupsAvailable') }}</p>
        </div>
      </div>
    </div>
