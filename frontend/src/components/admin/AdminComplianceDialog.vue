<template>
  ...
  >
    <div class="space-y-5">
      <div class="rounded-xl bg-amber-50 p-4 text-sm text-amber-900 ring-1 ring-inset ring-amber-200 dark:bg-amber-500/10 dark:text-amber-100 dark:ring-amber-500/30">
      ...
      <div class="grid gap-4 md:grid-cols-[minmax(0,1fr)_240px]">
        <section class="card max-h-[46vh] min-h-[320px] overflow-y-auto p-5">
          <div class="legal-document-content" v-html="renderedDocument"></div>
        </section>

        <aside class="card-inset space-y-3 p-4 text-sm">
          <div>
            <p class="text-xs font-medium uppercase tracking-[0.03em] text-gray-500 dark:text-dark-400">
              {{ t('adminCompliance.version') }}
            </p>
            <p class="tabular mt-1 break-all font-mono text-gray-900 dark:text-white">
      ...
      <div class="space-y-3">
        ...
        <div class="card-inset px-3 py-2 font-mono text-sm text-gray-900 dark:text-dark-100">
          {{ expectedPhrase }}
        </div>
        <Input
...
.legal-document-content :deep(h1) {
  @apply mb-4 text-2xl font-bold tracking-[-0.022em] text-gray-950 dark:text-white;
}

.legal-document-content :deep(h2) {
  @apply mb-3 mt-6 text-xl font-semibold tracking-[-0.014em] text-gray-900 dark:text-white;
}
