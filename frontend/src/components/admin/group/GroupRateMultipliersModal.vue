<template>
  <BaseDialog :show="show" :title="t('admin.groups.rateMultipliersTitle')" width="wide" @close="handleClose">
    <div v-if="group" class="space-y-4">
      <!-- 分组信息 -->
      <div class="flex flex-wrap items-center gap-3 rounded-xl bg-gray-50 px-4 py-2.5 text-sm dark:bg-dark-700/60">
      ...
      <!-- 操作区 -->
      <div class="hairline rounded-xl border p-3">
        <!-- 添加用户 -->
        ...
        <div class="flex items-end gap-2">
          <div class="relative flex-1">
            ...
            <div
              ...
              class="material-glass pop-origin-top scroll-edge-fade absolute left-0 right-0 top-full z-10 mt-1 max-h-48 animate-scale-in overflow-y-auto rounded-xl shadow-[var(--shadow-pop)]"
            >
              <button
                v-for="user in searchResults"
                :key="user.id"
                type="button"
                class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-sm transition-colors hover:bg-black/[0.04] dark:hover:bg-white/[0.06]"
          ...
          <button
            type="button"
            class="btn btn-primary press-feedback shrink-0"
        ...
        <!-- 批量调整 + 全部清空 -->
        <div v-if="localEntries.length > 0" class="hairline mt-3 flex items-center gap-3 border-t pt-3">
          ...
          <div class="flex items-center gap-1.5">
            ...
            <input
              ...
              min="0"
              autocomplete="off"
              class="hide-spinner w-20 rounded-lg border border-gray-200 bg-white px-2 py-1 text-center text-sm transition-colors focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/25 dark:border-dark-500 dark:bg-dark-700 dark:focus:border-primary-500"
              placeholder="0.5"
            />
            <button
              type="button"
              class="btn btn-primary btn-sm press-feedback shrink-0 px-2.5 py-1 text-xs"
          ...
          <div class="ml-auto">
            <button
              type="button"
              class="press-feedback rounded-full border border-red-200 bg-red-50 px-3 py-1.5 text-sm font-medium text-red-600 transition-colors hover:bg-red-100 dark:border-red-800 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/40"
              @click="clearAllLocal"
            >
      ...
      <div v-else>
        ...
        <div v-else>
          <!-- 表格 -->
          <div class="hairline overflow-hidden rounded-xl border">
            <div class="max-h-[420px] overflow-auto">
              <table class="w-full min-w-max text-sm">
                <thead class="sticky top-0 z-[1]">
                  <tr class="hairline border-b bg-gray-50/80 backdrop-blur dark:bg-dark-700/80">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-dark-600">
                  <tr
                    ...
                    :key="entry.user_id"
                    class="transition-colors hover:bg-black/[0.03] dark:hover:bg-white/[0.04]"
                  >
                    ...
                    <td class="whitespace-nowrap px-3 py-2">
                      <input
                        ...
                        class="hide-spinner w-20 rounded-lg border border-gray-200 bg-white px-2 py-1 text-center text-sm font-medium transition-colors focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/25 dark:border-dark-500 dark:bg-dark-700 dark:focus:border-primary-500"
                    ...
                    <td class="px-2 py-2">
                      <button
                        type="button"
                        class="press-feedback rounded-lg p-1 text-gray-400 transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400"
      ...
      <!-- 底部操作栏 -->
      <div class="hairline flex items-center gap-3 border-t pt-4">
        <!-- 左侧：未保存提示 + 撤销 -->
        ...
        <div class="ml-auto flex items-center gap-3">
          <button type="button" class="btn btn-secondary btn-sm press-feedback px-4 py-1.5" @click="handleClose">
            {{ t('common.close') }}
          </button>
          <button
            v-if="isDirty"
            type="button"
            class="btn btn-primary btn-sm press-feedback px-4 py-1.5"
            :disabled="saving"
            @click="handleSave"
          >
