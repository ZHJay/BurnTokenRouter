  ...
  >
    <form id="refund-form" @submit.prevent="handleSubmit" class="space-y-4">
      ...
      <div
        ...
        class="rounded-xl bg-violet-50 p-3 ring-1 ring-inset ring-violet-200 dark:bg-violet-900/20 dark:ring-violet-800"
      >
        ...
        <div v-if="order?.refund_requested_at" class="mt-2 flex justify-between text-sm">
          ...
          <span class="tabular text-violet-800 dark:text-violet-200">{{ formatDateTime(order.refund_requested_at) }}</span>
        </div>
      ...
      <div class="card-inset p-3">
        <div class="flex justify-between text-sm">
          ...
          <span class="tabular font-mono text-gray-900 dark:text-white">#{{ order?.id }}</span>
        </div>
        <div class="mt-1 flex justify-between text-sm">
          <span class="text-gray-500 dark:text-gray-400">{{ t('payment.orders.creditedAmount') }}</span>
          <span class="tabular font-semibold text-gray-900 dark:text-white">{{ creditedAmountSymbol }}{{ order?.amount?.toFixed(2) }}</span>
        </div>
        <div class="mt-1 flex justify-between text-sm">
          <span class="text-gray-500 dark:text-gray-400">{{ t('payment.orders.payAmount') }}</span>
          <span class="tabular font-semibold text-gray-900 dark:text-white">{{ paymentAmountSymbol }}{{ order?.pay_amount?.toFixed(2) }}</span>
        </div>
        <div v-if="actuallyRefunded > 0" class="mt-1 flex justify-between text-sm">
          ...
          <span class="tabular font-semibold text-red-600 dark:text-red-400">{{ creditedAmountSymbol }}{{ actuallyRefunded.toFixed(2) }}</span>
        </div>
      </div>
      ...
      <div>
        ...
        <div v-if="form.deduct_balance && userBalance != null" class="mt-3 grid grid-cols-2 gap-3">
          <div class="card-inset p-3 text-sm">
            <div class="text-gray-500 dark:text-gray-400">{{ t('payment.admin.userBalance') }}</div>
            <div class="tabular mt-1 font-semibold text-gray-900 dark:text-white">{{ creditedAmountSymbol }}{{ userBalance.toFixed(2) }}</div>
          </div>
          <div class="card-inset p-3 text-sm">
            <div class="text-gray-500 dark:text-gray-400">{{ t('payment.admin.orderAmount') }}</div>
            <div class="tabular mt-1 font-semibold text-gray-900 dark:text-white">{{ creditedAmountSymbol }}{{ order?.amount?.toFixed(2) }}</div>
          </div>
        </div>
        ...
        <div
          ...
          class="mt-2 rounded-xl bg-amber-50 p-3 text-sm text-amber-700 ring-1 ring-inset ring-amber-200 dark:bg-amber-900/20 dark:text-amber-300 dark:ring-amber-800"
        >
        ...
        <div
          ...
          class="mt-2 rounded-xl bg-[var(--accent-tint)] p-3 text-sm text-blue-700 shadow-[inset_0_0_0_0.5px_var(--hairline)] dark:text-blue-300"
        >
      ...
      <div
        v-if="warning"
        class="rounded-xl bg-yellow-50 p-3 text-sm text-yellow-700 ring-1 ring-inset ring-yellow-200 dark:bg-yellow-900/20 dark:text-yellow-300 dark:ring-yellow-800"
      >
        {{ warning }}
      </div>
      ...
      <div v-if="requireForce" class="flex items-center gap-2">
        <input
          ...
          v-model="form.force"
          type="checkbox"
          class="h-4 w-4 rounded-[5px] accent-[color:var(--sys-red-text)] focus-visible:outline-none focus-visible:shadow-[0_0_0_3.5px_rgb(255_59_48_/_0.18),0_0_0_1px_var(--sys-red-text)]"
        />
