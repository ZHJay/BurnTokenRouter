  ...
  <div class="space-y-4">
    ...
    <DataTable :columns="columns" :data="orders" :loading="loading">
      <template #cell-id="{ value }">
        <span class="tabular font-mono text-sm">#{{ value }}</span>
      </template>

      <template #cell-user_id="{ value }">
        <span class="tabular text-sm text-gray-600 dark:text-gray-400">#{{ value }}</span>
      </template>

      <template #cell-pay_amount="{ value, row }">
        <div class="text-sm">
          <span class="tabular font-semibold text-gray-900 dark:text-white">{{ paymentAmountSymbol(row) }}{{ value.toFixed(2) }}</span>
          <span v-if="row.fee_rate > 0" class="tabular ml-1 text-xs text-gray-400" :title="t('payment.orders.fee') + ': ' + row.fee_rate + '%'">
            ({{ row.fee_rate }}%)
          </span>
          <div v-if="row.amount !== row.pay_amount" class="tabular text-xs text-gray-500">
      ...
      <template #cell-created_at="{ value }">
        <span class="tabular text-xs text-gray-500 dark:text-gray-400">{{ formatDateTime(value) }}</span>
      </template>
      ...
      <template #cell-actions="{ row }">
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-gray-500 transition-[background-color,color,transform] duration-fast ease-apple-out hover:bg-yellow-50 hover:text-yellow-600 focus-visible:outline-none focus-visible:ring-[3.5px] focus-visible:ring-[rgb(255_149_0_/_0.24)] active:scale-[0.96] dark:hover:bg-yellow-900/20 dark:hover:text-yellow-400"
          >
            <Icon name="x" size="sm" />
          ...
          <button
            ...
            class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-gray-500 transition-[background-color,color,transform] duration-fast ease-apple-out hover:bg-blue-50 hover:text-blue-600 focus-visible:outline-none focus-visible:ring-[3.5px] focus-visible:ring-[var(--accent-tint-strong)] active:scale-[0.96] dark:hover:bg-blue-900/20 dark:hover:text-blue-400"
          >
