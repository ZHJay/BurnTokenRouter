<template>
  <Teleport to="body">
    <div v-if="show && position">
      ...
      <div
        class="action-menu-content dropdown fixed z-[9999] w-52 origin-top"
      ...
      >
        <div>
          <template v-if="account">
            ...
            <button @click="$emit('schedule', account); $emit('close')" class="dropdown-item w-full">
              <Icon name="clock" size="sm" class="text-orange-500" />
              {{ t('admin.scheduledTests.schedule') }}
            </button>
            <button v-if="canDuplicate" @click="$emit('duplicate', account); $emit('close')" class="dropdown-item w-full">
            ...
            <button v-if="isOpenAIOAuthParent" @click="$emit('create-spark-shadow', account); $emit('close')" class="dropdown-item w-full text-amber-600 dark:text-amber-400">
              <Icon name="sparkles" size="sm" />
              {{ t('admin.accounts.createSparkShadow') }}
            </button>
            <button v-if="supportsPrivacy" @click="$emit('set-privacy', account); $emit('close')" class="dropdown-item w-full text-emerald-600 dark:text-emerald-400">
              <Icon name="shield" size="sm" />
              {{ t('admin.accounts.setPrivacy') }}
            </button>
            <div v-if="hasRecoverableState" class="my-1 h-[0.5px] bg-[var(--separator)]"></div>
            <button v-if="hasRecoverableState" @click="$emit('recover-state', account); $emit('close')" class="dropdown-item w-full text-emerald-600 dark:text-emerald-400">
              <Icon name="sync" size="sm" />
              {{ t('admin.accounts.recoverState') }}
            </button>
            <button v-if="hasQuotaLimit" @click="$emit('reset-quota', account); $emit('close')" class="dropdown-item w-full text-teal-600 dark:text-teal-400">
              <Icon name="refresh" size="sm" />
