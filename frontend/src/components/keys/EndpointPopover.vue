...
<template>
  <div v-if="allEndpoints.length > 0" class="flex flex-wrap gap-2">
    <div
      ...
      :key="index"
      class="flex items-center gap-1.5 rounded-full border hairline bg-white/80 px-3 py-1.5 text-xs shadow-sm backdrop-blur-sm transition-all duration-200 ease-[cubic-bezier(0.32,0.72,0,1)] hover:border-primary-200 hover:shadow-md dark:bg-dark-800/70 dark:hover:border-primary-700"
    >
      <span class="font-medium text-gray-600 dark:text-gray-300">{{ item.name }}</span>
      <span
        v-if="item.isDefault"
        class="rounded-full bg-primary-50 px-1.5 py-px text-[10px] font-medium leading-tight text-primary-600 dark:bg-primary-900/30 dark:text-primary-400"
      ...
      <div class="group/endpoint relative flex items-center gap-1.5">
        <div
          class="material-glass pop-origin-top pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 w-max max-w-[24rem] -translate-x-1/2 translate-y-1 scale-95 rounded-2xl px-3 py-2.5 text-left opacity-0 shadow-[var(--shadow-pop)] transition-all duration-200 ease-[cubic-bezier(0.32,0.72,0,1)] group-hover/endpoint:translate-y-0 group-hover/endpoint:scale-100 group-hover/endpoint:opacity-100 group-focus-within/endpoint:translate-y-0 group-focus-within/endpoint:scale-100 group-focus-within/endpoint:opacity-100"
        >
          <p
          ...
          <div class="material-glass absolute left-1/2 top-full h-3 w-3 -translate-x-1/2 -translate-y-1/2 rotate-45 rounded-[3px]"></div>
        </div>
        ...
        <button
          type="button"
          class="press-feedback rounded-full p-0.5 transition-colors"
        ...
        <a
          ...
          class="press-feedback rounded-full p-0.5 text-gray-400 transition-colors hover:text-amber-500 dark:text-gray-500 dark:hover:text-amber-400"
