<template>
  <DataTable :columns="columns" :data="orders" :loading="loading">
    ...
    <template #cell-out_trade_no="{ value }">
      <span class="tabular text-sm text-gray-900 dark:text-white">{{ value }}</span>
    </template>
    ...
    <template #cell-pay_amount="{ value, row }">
      <div class="text-sm">
        <span class="tabular font-medium text-gray-900 dark:text-white">{{ paymentAmountSymbol(row) }}{{ value.toFixed(2) }}</span>
    ...
    <template #cell-created_at="{ value }">
      <span class="tabular text-xs text-gray-500 dark:text-gray-400">{{ formatDate(value) }}</span>
    </template>
