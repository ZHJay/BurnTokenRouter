<template>
  <div
    :class="[
      ...
      'transition duration-fast ease-spring hover:-translate-y-0.5 hover:shadow-elev-3',
      borderClass,
    ...
    <div class="flex flex-1 flex-col p-4">
      ...
      <div class="mb-3 flex items-start justify-between gap-2">
        ...
        <div class="shrink-0 text-right">
          <div class="flex items-baseline gap-1">
            ...
            <span :class="['tabular text-2xl font-semibold tracking-[-0.026em]', textClass]">{{ plan.price }}</span>
          ...
          <div v-if="plan.original_price" class="mt-0.5 flex items-center justify-end gap-1.5">
            <span class="tabular text-xs text-gray-400 line-through dark:text-dark-500">{{ planCurrencySymbol }}{{ plan.original_price }}<template v-if="plan.currency"> {{ plan.currency }}</template></span>
            <span :class="['tabular rounded-md px-1 py-0.5 text-[10px] font-semibold', discountClass]">{{ discountText }}</span>
          </div>
        </div>
      </div>

      <!-- Group quota info (compact) -->
      <!-- Not .card-inset: that fill is --surface-secondary (#2c2c2e), the same
           colour as this card's dark:bg-dark-800, so the nested panel would
           disappear in dark mode. Keep the lighter fill, add the hairline. -->
      <div
        class="mb-3 grid grid-cols-2 gap-x-3 gap-y-1 rounded-lg bg-gray-50 px-3 py-2 text-xs dark:bg-dark-700"
        style="box-shadow: inset 0 0 0 0.5px var(--hairline)"
      >
        <div class="flex items-center justify-between">
          <span class="text-gray-400 dark:text-dark-500">{{ t('payment.planCard.rate') }}</span>
          <span class="tabular font-medium text-gray-700 dark:text-gray-300">{{ rateDisplay }}</span>
        </div>
        <div v-if="hasPeakRate" class="col-span-2 flex items-center justify-between gap-2">
          ...
          <span class="tabular text-right font-medium text-amber-700 dark:text-amber-300">{{ peakRateDisplay }}</span>
        </div>
        <div v-if="plan.daily_limit_usd != null" class="flex items-center justify-between">
          <span class="text-gray-400 dark:text-dark-500">{{ t('payment.planCard.dailyLimit') }}</span>
          <span class="tabular font-medium text-gray-700 dark:text-gray-300">${{ plan.daily_limit_usd }}</span>
        </div>
        <div v-if="plan.weekly_limit_usd != null" class="flex items-center justify-between">
          <span class="text-gray-400 dark:text-dark-500">{{ t('payment.planCard.weeklyLimit') }}</span>
          <span class="tabular font-medium text-gray-700 dark:text-gray-300">${{ plan.weekly_limit_usd }}</span>
        </div>
        <div v-if="plan.monthly_limit_usd != null" class="flex items-center justify-between">
          <span class="text-gray-400 dark:text-dark-500">{{ t('payment.planCard.monthlyLimit') }}</span>
          <span class="tabular font-medium text-gray-700 dark:text-gray-300">${{ plan.monthly_limit_usd }}</span>
        </div>
        ...
        <div v-if="modelScopeLabels.length > 0" class="col-span-2 flex items-center justify-between">
          ...
          <div class="flex flex-wrap justify-end gap-1">
            <span v-for="scope in modelScopeLabels" :key="scope"
              class="rounded-md bg-gray-200 px-1.5 py-0.5 text-[10px] font-medium text-gray-600 dark:bg-dark-600 dark:text-gray-300">
              {{ scope }}
            </span>
          </div>
        </div>
      ...
      <button
        ...
        :class="[
          ...
          'transition duration-fast ease-apple-out active:scale-[0.96]',
