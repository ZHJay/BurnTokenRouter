<template>
  ...
  >
    <form id="provider-form" @submit.prevent="handleSave" class="space-y-4">
          ...
          <div class="flex gap-1.5">
            <button
              ...
              :class="[
                ...
                'transition duration-fast ease-apple-out active:scale-[0.96] focus-visible:outline-none',
                'focus-visible:ring-[3.5px] focus-visible:ring-[color:var(--accent-tint-strong)]',
                form.payment_mode === mode.value
                  ? 'border-primary-500 bg-primary-500 text-white shadow-elev-1'
      ...
      <div v-if="form.provider_key === 'easypay'" class="card-inset space-y-3 p-3">
        ...
        <div v-if="easyPayCustomMethods.length" class="space-y-2">
          ...
          >
            ...
            <button
              type="button"
              class="rounded-lg border border-red-200 px-2.5 py-2 text-xs font-medium text-red-600 transition duration-fast ease-apple-out hover:bg-red-50 active:scale-[0.96] focus-visible:outline-none focus-visible:ring-[3.5px] focus-visible:ring-[color:rgb(255_59_48_/_0.24)] dark:border-red-800/60 dark:text-red-300 dark:hover:bg-red-900/20"
      ...
      <!-- Config fields -->
      <div class="pt-4" style="box-shadow: inset 0 0.5px 0 var(--separator)">
        <div class="mb-3 flex items-center gap-2">
          ...
          <HelpTooltip v-if="paymentGuide" trigger="click" width-class="w-80">
            <template #trigger>
              <button
                type="button"
                class="inline-flex h-5 w-5 items-center justify-center rounded-full border border-gray-300 text-[11px] font-semibold text-gray-400 transition duration-fast ease-apple-out hover:border-primary-500 hover:text-primary-600 active:scale-[0.96] focus-visible:outline-none focus-visible:ring-[3.5px] focus-visible:ring-[color:var(--accent-tint-strong)] dark:border-dark-500 dark:text-gray-500 dark:hover:border-primary-400 dark:hover:text-primary-400"
        ...
        <div v-if="callbackPaths" class="mt-4 space-y-3">
          <div v-if="callbackPaths.notifyUrl">
            ...
            <div class="flex">
              ...
              <span class="inline-flex items-center whitespace-nowrap rounded-r-lg bg-gray-50 px-3 text-xs text-gray-500 dark:bg-dark-700 dark:text-gray-400" style="box-shadow: inset 0 0 0 1px var(--separator)">{{ callbackPaths.notifyUrl }}</span>
            </div>
          </div>
          <div v-if="callbackPaths.returnUrl">
            ...
            <div class="flex">
              ...
              <span class="inline-flex items-center whitespace-nowrap rounded-r-lg bg-gray-50 px-3 text-xs text-gray-500 dark:bg-dark-700 dark:text-gray-400" style="box-shadow: inset 0 0 0 1px var(--separator)">{{ callbackPaths.returnUrl }}</span>
            </div>
          </div>
        </div>

        <!-- 服务商 Webhook 提示 -->
        <div v-if="providerWebhookUrl" class="mt-3 rounded-lg bg-blue-50 p-3 dark:bg-blue-900/20" style="box-shadow: inset 0 0 0 0.5px var(--accent-tint-strong)">
          <p class="text-xs text-blue-700 dark:text-blue-300">
            {{ t(providerWebhookHint) }}
          </p>
          <code class="mt-1 block break-all rounded-md bg-blue-100 px-2 py-1 font-mono text-xs text-blue-800 dark:bg-blue-900/40 dark:text-blue-200">
            {{ providerWebhookUrl }}
          </code>
      ...
      <div v-if="limitableTypes.length" class="pt-4" style="box-shadow: inset 0 0.5px 0 var(--separator)">
        <button type="button" @click="limitsExpanded = !limitsExpanded" class="flex w-full items-center justify-between">
          ...
          <svg :class="['h-4 w-4 text-gray-400 transition-transform duration-fast ease-spring', limitsExpanded && 'rotate-180']" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
        </button>
