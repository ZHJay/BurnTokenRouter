<template>
  ...
  >
    ...
    >
      ...
      <div
        class="card-inset p-3 text-xs text-gray-500 dark:text-dark-400"
      >
        {{ t('admin.accounts.crsUpdateBehaviorNote') }}
      </div>
      <div
        class="rounded-lg bg-amber-500/10 p-3 text-xs text-amber-600 shadow-[inset_0_0_0_0.5px_rgb(255_149_0_/_0.28)] dark:text-amber-400"
      >
    ...
    <div v-else-if="currentStep === 'preview' && previewResult" class="space-y-4">
      ...
      <div v-if="previewResult.new_accounts.length">
        ...
        <div
          class="card-inset max-h-48 overflow-auto p-2"
        >
          <label
            v-for="acc in previewResult.new_accounts"
            :key="acc.crs_account_id"
            class="flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 transition-transform duration-instant ease-apple-out hover:bg-[var(--surface-hover)] active:scale-[0.98]"
          >
            <input
              type="checkbox"
      ...
      <div
        ...
        class="card-inset p-4 text-center text-sm text-gray-500 dark:text-dark-400"
      >
    ...
    <div v-else-if="currentStep === 'result' && result" class="space-y-4">
      ...
      >
        ...
        <div v-if="errorItems.length" class="mt-2">
          ...
          <div
            class="code-block mt-2 max-h-48 overflow-auto p-3 text-xs"
          >
