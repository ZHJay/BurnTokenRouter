<template>
  <div class="flex items-center gap-2">
    ...
    <div v-if="isRateLimited" class="flex flex-col items-center gap-1">
      ...
      <span class="tabular text-[11px] tracking-[0.01em] text-gray-400 dark:text-gray-500">{{ rateLimitResumeText }}</span>
    </div>
    ...
    <div v-else-if="isOverloaded" class="flex flex-col items-center gap-1">
      ...
      <span class="tabular text-[11px] tracking-[0.01em] text-gray-400 dark:text-gray-500">{{ overloadCountdown }}</span>
    </div>
    ...
    <template v-else>
      <button
        ...
        :class="[
          'badge text-xs',
          statusClass,
          'cursor-pointer transition-transform duration-instant ease-apple-out active:scale-[0.96]'
        ]"
    ...
    <div v-if="hasError && account.error_message" class="group/error relative">
      ...
      <!-- Tooltip - 向下显示 -->
      <div
        class="invisible absolute left-0 top-full z-[100] mt-1.5 min-w-[200px] max-w-[300px] rounded-lg bg-gray-800 px-3 py-2 text-xs text-white opacity-0 shadow-elev-3 transition-all duration-fast ease-apple-out group-hover/error:visible group-hover/error:opacity-100 dark:bg-gray-900"
      >
    ...
    <div v-if="isRateLimited" class="group relative">
      <span
        class="badge tabular rounded-md bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"
      >
      ...
      <!-- Tooltip -->
      <div
        class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 w-56 -translate-x-1/2 whitespace-normal rounded-lg bg-gray-900 px-3 py-2 text-center text-xs leading-relaxed text-white opacity-0 shadow-elev-3 transition-opacity group-hover:opacity-100 dark:bg-gray-700"
      >
    ...
    >
      <div v-for="item in activeModelStatuses" :key="`${item.kind}-${item.model}`" class="group relative mb-1 break-inside-avoid">
        ...
        <span
          ...
          class="badge rounded-md bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"
        >
          <Icon name="exclamationTriangle" size="xs" :stroke-width="2" />
          {{ t('admin.accounts.status.creditsExhausted') }}
          <span class="tabular text-[10px] opacity-70">{{ formatCountdown(item.reset_at) }}</span>
        </span>
        <!-- 正在走积分（模型限流但积分可用）-->
        <span
          v-else-if="item.kind === 'credits_active'"
          class="badge rounded-md bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400"
        >
          <span>⚡</span>
        ...
        <span
          v-else
          class="badge rounded-md bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400"
        >
          <Icon name="exclamationTriangle" size="xs" :stroke-width="2" />
          {{ formatScopeName(item.model) }}
          <span class="tabular text-[10px] opacity-70">{{ formatCountdown(item.reset_at) }}</span>
        ...
        <!-- Tooltip -->
        <div
          class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 w-max max-w-[320px] -translate-x-1/2 whitespace-nowrap rounded-lg bg-gray-900 px-3 py-2 text-center text-xs leading-relaxed text-white opacity-0 shadow-elev-3 transition-opacity group-hover:opacity-100 dark:bg-gray-700"
        >
          {{
    ...
    <div v-if="isOverloaded" class="group relative">
      <span
        class="badge tabular rounded-md bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"
      >
        <Icon name="exclamationTriangle" size="xs" :stroke-width="2" />
        529
      </span>
