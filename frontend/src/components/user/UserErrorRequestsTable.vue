    ...
    <div class="card flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      >
        ...
        <template #cell-key_name="{ row }">
          <div class="text-sm">
            ...
            <span
              ...
              class="badge badge-danger ml-1 text-[10px] leading-tight"
        ...
        <template #cell-status="{ row }">
          <span class="badge tabular" :class="statusClass(row.status_code)">
            {{ row.status_code || '-' }}
          </span>
        </template>
        ...
        <template #cell-group="{ row }">
          <span
            v-if="row.group_name"
            class="badge bg-indigo-100 text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200"
          >{{ row.group_name }}</span>
          <span v-else class="text-sm text-gray-400 dark:text-gray-500">-</span>
        </template>
