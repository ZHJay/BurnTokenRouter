<template>
  <div>
    ...
    <button
      @click="openModal"
      class="btn btn-ghost btn-icon relative flex h-9 w-9"
    ...
    <Teleport to="body">
      <Transition name="modal-fade">
        <div
          v-if="isModalOpen"
          class="bell-scrim fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto p-4 pt-[8vh]"
          @click="closeModal"
        >
          <div
            class="glass-thin w-full max-w-[620px] overflow-hidden rounded-3xl shadow-glass-edge"
            @click.stop
          >
            <!-- Header: flat accent tint instead of a gradient wash -->
            <div class="hairline-bottom relative overflow-hidden bg-blue-500/[0.06] px-6 py-5 dark:bg-blue-400/[0.08]">
              <div class="relative z-10 flex items-start justify-between">
                <div>
                  <div class="flex items-center gap-2">
                    <div class="stat-icon stat-icon-primary h-8 w-8 text-base">
                      <Icon name="bell" size="sm" />
                    </div>
                    <h2 class="text-lg font-semibold tracking-[-0.014em] text-gray-900 dark:text-white">
                      {{ t('announcements.title') }}
                    </h2>
                  </div>
                  <p v-if="unreadCount > 0" class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                    <span class="tabular font-semibold text-blue-600 dark:text-blue-400">{{ unreadCount }}</span>
            ...
            <div class="max-h-[65vh] overflow-y-auto">
              ...
              <div v-if="loading" class="flex items-center justify-center py-16">
                <div class="relative">
                  <div class="spinner h-12 w-12 border-4 text-blue-600 dark:text-blue-400"></div>
              ...
              <div v-else-if="announcements.length > 0">
                <div
                  ...
                  :key="item.id"
                  class="hairline-bottom group relative flex cursor-pointer items-center gap-4 px-6 py-4 transition-colors duration-fast ease-apple-out last:shadow-none hover:bg-gray-500/[0.04] active:scale-[0.98] dark:hover:bg-white/[0.05]"
                ...
                >
                  ...
                  <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center">
                    <div
                      v-if="!item.read_at"
                      class="relative flex h-10 w-10 items-center justify-center rounded-lg bg-blue-600 text-white shadow-elev-2"
                    >
                      <!-- Pulse ring -->
                      <span class="absolute inline-flex h-full w-full animate-ping rounded-lg bg-blue-400 opacity-75"></span>
                      <!-- Icon -->
                  ...
                  <div class="flex min-w-0 flex-1 items-center justify-between gap-4">
                    <div class="min-w-0 flex-1">
                      ...
                      <div class="mt-1 flex items-center gap-2">
                        <time class="tabular text-xs text-gray-500 dark:text-gray-400">
                          {{ formatRelativeTime(item.created_at) }}
                        </time>
                        <span
                          v-if="!item.read_at"
                          class="badge badge-primary px-1.5"
                        >
                    ...
                    <div class="flex-shrink-0">
                      <svg
                        class="h-5 w-5 text-gray-400 transition-transform duration-fast ease-apple-out group-hover:translate-x-1 dark:text-gray-600"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        stroke-width="2"
                  ...
                  <div
                    v-if="!item.read_at"
                    class="absolute left-0 top-0 h-full w-1 bg-blue-600"
                  ></div>
                </div>
              </div>
              ...
              <div v-else class="flex flex-col items-center justify-center py-16">
                <div class="relative mb-4">
                  <div class="flex h-20 w-20 items-center justify-center rounded-full bg-gray-100 dark:bg-dark-800">
    ...
    <Teleport to="body">
      <Transition name="modal-fade">
        <div
          ...
          class="bell-scrim fixed inset-0 z-[110] flex items-start justify-center overflow-y-auto p-4 pt-[6vh]"
          @click="closeDetail"
        >
          <div
            class="glass-thin w-full max-w-[780px] overflow-hidden rounded-3xl shadow-glass-edge"
            @click.stop
          >
            <!-- Header with Decorative Elements -->
            <div class="hairline-bottom relative overflow-hidden bg-blue-500/[0.07] px-8 py-6 dark:bg-blue-400/[0.09]">
              ...
              <div class="relative z-10 flex items-start justify-between gap-4">
                <div class="flex-1 min-w-0">
                  ...
                  <div class="mb-3 flex items-center gap-2">
                    <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-blue-600 text-white shadow-elev-2">
                      <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                    </div>
                    <div class="flex items-center gap-2">
                      <span class="badge badge-primary px-2.5 py-1">
                        {{ t('announcements.title') }}
                      </span>
                      <span
                        v-if="!selectedAnnouncement.read_at"
                        class="badge bg-blue-600 px-2.5 py-1 text-white shadow-elev-1"
                      >
                  ...
                  <!-- Title -->
                  <h2 class="mb-3 text-2xl font-bold leading-tight tracking-[-0.022em] text-gray-900 dark:text-white">
                  ...
                  <div class="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400">
                    <div class="flex items-center gap-1.5">
                      ...
                      <time class="tabular">{{ formatRelativeWithDateTime(selectedAnnouncement.created_at) }}</time>
                    </div>
                    <div class="flex items-center gap-1.5">
                      <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <button
                  @click="closeDetail"
                  class="btn btn-ghost btn-icon h-10 w-10 flex-shrink-0"
                  :aria-label="t('common.close')"
                >
                  <Icon name="x" size="md" />
                </button>
            ...
            <div class="max-h-[60vh] overflow-y-auto px-8 py-8">
              <!-- Content with decorative border -->
              <div class="relative">
                <!-- Decorative left border -->
                <div class="absolute left-0 top-0 bottom-0 w-1 rounded-full bg-blue-600"></div>
