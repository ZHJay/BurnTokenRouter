...
describe('MonitorQuotaView', () => {
  ...
  it('maps tier window/label tokens through i18n and colors utilization', () => {
    ...
    const rows = wrapper.findAll('[data-testid="monitor-quota-view"] .quota-row')
    ...
    const bars = wrapper.findAll('.meter .fill')
    // 统一消费 v3 meter：≥90 high / ≥75 warn / 其余默认蓝色。
    expect(bars).toHaveLength(3)
    expect(bars[0].classes()).not.toContain('warn')
    expect(bars[0].classes()).not.toContain('high')
    expect(bars[1].classes()).toContain('warn')
    expect(bars[2].classes()).toContain('high')
  })
  ...
  it('shows the plan level badge and multi-currency balances', () => {
    ...
    // 余额为 0 用红色警示
    expect(wrapper.find('[data-balance-state="low"]').exists()).toBe(true)
  })
