<template>
  <div class="flex min-h-screen items-center justify-center bg-[var(--surface-bg)] p-4">
    <div
      class="material-glass w-full max-w-md space-y-4 rounded-[24px] p-6 shadow-[var(--shadow-pop)]"
    >
      <!-- Amount + Order ID -->
      <div v-if="amount" class="text-center">
        <p class="text-3xl font-bold tabular-nums" :style="{ color: methodColor }">¥{{ amount }}</p>
      ...
      <div v-if="error" class="space-y-3">
        <div
          class="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-600 dark:border-red-700 dark:bg-red-900/30 dark:text-red-400"
        >
          {{ error }}
        </div>
        <button
          class="press-feedback w-full text-sm underline dark:text-blue-400 dark:hover:text-blue-300"
      ...
      <div v-else-if="success" class="space-y-3 py-4 text-center">
        <div class="text-5xl text-green-600 dark:text-green-400 animate-scale-in">✓</div>
        <p class="text-sm text-gray-500 dark:text-slate-400">{{ t('payment.result.success') }}</p>
        <button
          class="press-feedback text-sm underline dark:text-blue-400 dark:hover:text-blue-300"
          :style="{ color: methodColor }"
          @click="closeWindow"
        >
          {{ t('common.close') }}
