<template>
  <AppLayout>
    <div class="mx-auto flex max-w-md flex-col items-center space-y-6 py-8">
      <h2 class="text-xl font-semibold tracking-[-0.01em] text-gray-900 dark:text-white">
        {{ qrUrl ? scanTitle : t('payment.qr.payInNewWindow') }}
      </h2>
      <div v-if="qrUrl" class="rounded-[24px] bg-white p-6 shadow-[var(--shadow-pop)] dark:bg-dark-800">
      ...
      <div v-if="expired" class="text-center">
        ...
        <button class="btn btn-primary press-feedback mt-4" @click="router.push('/purchase')">{{ t('payment.result.backToRecharge') }}</button>
      </div>
      ...
      <!-- Cancel button -->
      <button v-if="!expired && orderId" class="btn btn-secondary press-feedback w-full" :disabled="cancelling" @click="handleCancel">
