<template>
  <div class="min-h-screen bg-[color:var(--surface-bg)] px-4 py-10">
    <div class="mx-auto max-w-2xl">
      <div class="card p-6">
        <h1 class="text-lg font-semibold tracking-tight text-[color:var(--text-primary)] dark:text-white">
          {{ callbackTitleText }}
        </h1>
        ...
        <div
          v-else
          class="mt-6 rounded-[14px] border hairline bg-[color:var(--fill)] p-4"
        >
          <p class="text-sm text-[color:var(--text-primary)] dark:text-gray-300">
            {{ errorMessage }}
          </p>
          <button
            class="btn btn-primary press-feedback mt-4"
            type="button"
