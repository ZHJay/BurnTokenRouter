<template>
  <AppLayout>
    <TablePageLayout>
      <template #filters>
        <div class="toolbar items-start justify-between">
          <AccountTableFilters
            ...
            <template #after>
              ...
              <div class="relative" ref="autoRefreshDropdownRef">
                ...
                <div
                  ...
                  class="pop-card absolute right-0 z-50 mt-2 w-56 origin-top-right"
                >
                  <div class="p-2">
                    <button
              ...
              <div class="relative" ref="accountToolsDropdownRef">
                ...
                <Teleport to="body">
                  <div
                    ...
                    class="pop-card fixed z-[9999] origin-top-right overflow-hidden"
                  ...
                  >
                    <div class="overflow-y-auto p-2" :style="{ maxHeight: `${accountToolsDropdownPosition.maxHeight}px` }">
                      ...
                      <button class="pop-item" @click="openSyncFromCrs">
                        <span class="pop-icon bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-300">
                          <Icon name="sync" size="sm" />
                        </span>
                        <span class="flex-1 text-left">{{ t('admin.accounts.syncFromCrs') }}</span>
                      </button>
                      <button class="pop-item" @click="openImportData">
                        <span class="pop-icon bg-emerald-50 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-300">
                          <Icon name="upload" size="sm" />
                        </span>
                        <span class="flex-1 text-left">{{ t('admin.accounts.dataImport') }}</span>
                      </button>
                      <button class="pop-item" @click="openExportDataDialogFromMenu">
                        <span class="pop-icon bg-violet-50 text-violet-600 dark:bg-violet-900/30 dark:text-violet-300">
                      ...
                      <button class="pop-item" @click="openErrorPassthrough">
                        <span class="pop-icon bg-amber-50 text-amber-600 dark:bg-amber-900/30 dark:text-amber-300">
                          <Icon name="shield" size="sm" />
                        </span>
                        <span class="flex-1 text-left">{{ t('admin.errorPassthrough.title') }}</span>
                      </button>
                      <button class="pop-item" @click="openTLSFingerprintProfiles">
                        <span class="pop-icon bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-200">
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-select="{ row }">
            <input type="checkbox" :checked="isSelected(row.id)" @change="toggleSel(row.id)" class="rounded border-gray-300 accent-primary-500 focus:ring-primary-500" />
          </template>
              ...
              >
                <template #trigger>
                  <a
                    ...
                    class="strong border-b border-dotted border-gray-300 font-medium text-gray-900 dark:border-dark-600 dark:text-white"
                  >
                    {{ value }}
                  </a>
                </template>
              </HelpTooltip>
              <span v-else class="strong">{{ value }}</span>
              <span
                v-if="accountDisplayEmail(row)"
                class="muted truncate text-xs max-w-[200px]"
          ...
          <template #cell-rate_multiplier="{ row }">
            <span class="mono inline-flex items-center gap-1">
          ...
          <template #cell-last_used_at="{ value }">
            <span class="muted text-sm">{{ formatRelativeTime(value) }}</span>
          </template>
          <template #cell-created_at="{ value }">
            <span class="muted text-sm">{{ formatDateTime(value) }}</span>
          </template>
          <template #cell-expires_at="{ row, value }">
            <div class="flex flex-col items-start gap-1">
              <span class="muted text-sm">{{ formatExpiresAt(value) }}</span>
    ...
    <ConfirmDialog :show="showExportDataDialog" :title="t('admin.accounts.dataExport')" :message="t('admin.accounts.dataExportConfirmMessage')" :confirm-text="t('admin.accounts.dataExportConfirm')" :cancel-text="t('common.cancel')" @confirm="handleExportData" @cancel="showExportDataDialog = false">
      <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
        <input type="checkbox" class="h-4 w-4 rounded border-gray-300 accent-primary-500 focus:ring-primary-500" v-model="includeProxyOnExport" />
...
<style scoped>
/* 不透明浮出面板（契约：--glass-bg-strong + 纱幕压暗，不透出下层文字） */
.pop-card {
...
/* 行内 iOS 开关的聚焦态（全局 .switch 未带 focus 样式） */
