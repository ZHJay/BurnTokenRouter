    ...
    <TablePageLayout>
      <template #filters>
                  ...
                  <transition name="dropdown">
                    ...
                    <div class="overflow-y-auto" :style="{ maxHeight: `${accountToolsDropdownPosition.maxHeight}px` }">
                      ...
                      <button class="account-tools-menu-item" @click="openSyncFromCrs">
                        <span class="account-tools-menu-icon bg-blue-100/70 text-blue-600 dark:bg-blue-900/30 dark:text-blue-300">
                          <Icon name="sync" size="sm" />
                        </span>
                        <span class="flex-1 text-left">{{ t('admin.accounts.syncFromCrs') }}</span>
                      </button>
                      <button class="account-tools-menu-item" @click="openImportData">
                        <span class="account-tools-menu-icon bg-emerald-100/70 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-300">
                          <Icon name="upload" size="sm" />
                        </span>
                        <span class="flex-1 text-left">{{ t('admin.accounts.dataImport') }}</span>
                      </button>
                      <button class="account-tools-menu-item" @click="openExportDataDialogFromMenu">
                        <span class="account-tools-menu-icon bg-violet-100/70 text-violet-600 dark:bg-violet-900/30 dark:text-violet-300">
                        ...
                        <span
                          v-if="selIds.length"
                          class="badge badge-primary tabular"
                        >
                          {{ t('admin.accounts.selectedCount', { count: selIds.length }) }}
                        </span>
                      </button>

                      <div class="my-1.5 h-px" style="background-color: var(--separator)"></div>
                      <div class="px-2.5 py-1.5">
                        <div class="on-glass text-[11px] font-semibold uppercase tracking-[0.04em] text-gray-400 dark:text-gray-500">
                      ...
                      <button class="account-tools-menu-item" @click="openErrorPassthrough">
                        <span class="account-tools-menu-icon bg-amber-100/70 text-amber-600 dark:bg-amber-900/30 dark:text-amber-300">
                          <Icon name="shield" size="sm" />
                        </span>
                        <span class="flex-1 text-left">{{ t('admin.errorPassthrough.title') }}</span>
                      </button>
                      <button class="account-tools-menu-item" @click="openTLSFingerprintProfiles">
                        <span class="account-tools-menu-icon bg-slate-200/70 text-slate-600 dark:bg-slate-700 dark:text-slate-200">
                      ...
                      <div class="px-2.5 py-1.5">
                        <div class="flex items-center justify-between gap-3">
                          <span class="on-glass text-[11px] font-semibold uppercase tracking-[0.04em] text-gray-400 dark:text-gray-500">
        ...
        <div
          ...
          class="mt-2 flex items-center justify-between rounded-xl bg-amber-50 px-3 py-2 text-sm text-amber-800 shadow-elev-1 dark:bg-amber-900/20 dark:text-amber-200"
          style="box-shadow: inset 0 0 0 0.5px rgb(255 149 0 / 0.28), var(--shadow-1)"
        >
      ...
      <template #table>
        ...
        >
            ...
            <div class="flex min-w-0 flex-col gap-1">
              <div class="flex flex-wrap items-center gap-1">
                ...
                <span
                  ...
                  :class="['inline-block rounded-md px-1.5 py-0.5 text-[10px] font-[590] tracking-[0.008em]', getAntigravityTierClass(row)]"
                >
              ...
              <div
                ...
                :class="[
                  'inline-flex items-center gap-1.5 pl-0.5 text-[11px] font-[590] leading-4',
          ...
          <template #cell-proxy="{ row }">
            <div class="flex flex-col gap-1">
              ...
              <div v-if="row.proxy && row.proxy.expires_at" class="flex items-center gap-2 text-xs">
                <span class="tabular text-gray-600 dark:text-gray-300">{{ formatDateTime(row.proxy.expires_at) }}</span>
              ...
              <div v-if="row.proxy_fallback_origin_id" class="flex items-center gap-1">
                <span class="badge badge-warning" :title="t('admin.accounts.fallbackActiveTip', { origin: row.proxy_fallback_origin_name })">
                  {{ t('admin.accounts.fallbackActive') }}
                </span>
                <button class="btn btn-secondary btn-sm" @click="onRevertFallback(row)">{{ t('admin.accounts.revertProxy') }}</button>
              </div>
            </div>
          </template>
          <template #cell-rate_multiplier="{ row }">
            <span class="tabular inline-flex items-center gap-1 text-sm font-semibold tracking-[-0.006em] text-gray-700 dark:text-gray-300">
          ...
          <template #cell-priority="{ value }">
            <span class="tabular text-sm text-gray-700 dark:text-gray-300">{{ value }}</span>
          </template>
          ...
          <template #cell-scheduler_score="{ row }">
            <div v-if="getSchedulerScoreRows(row).length" class="tabular flex min-w-[7rem] flex-col gap-0.5 font-mono text-[11px] leading-4">
              <div
              ...
              >
                ...
                <span class="font-semibold">{{ formatSchedulerScore(score.base_score) }}</span>
                <span class="text-gray-300 dark:text-gray-600">/</span>
                <span class="font-semibold text-primary-700 dark:text-primary-300">{{ formatStickySchedulerScore(score) }}</span>
              </div>
            </div>
            <span v-else class="text-sm text-gray-400 dark:text-dark-500">-</span>
          </template>
          <template #cell-last_used_at="{ value }">
            <span class="tabular text-sm text-gray-500 dark:text-dark-400">{{ formatRelativeTime(value) }}</span>
          </template>
          <template #cell-created_at="{ value }">
            <span class="tabular text-sm text-gray-500 dark:text-dark-400">{{ formatDateTime(value) }}</span>
          </template>
          <template #cell-expires_at="{ row, value }">
            <div class="flex flex-col items-start gap-1">
              <span class="tabular text-sm text-gray-500 dark:text-dark-400">{{ formatExpiresAt(value) }}</span>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button @click="handleEdit(row)" class="row-action hover:text-primary-600 dark:hover:text-primary-400">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                <span class="text-xs">{{ t('common.edit') }}</span>
              </button>
              <button @click="handleDelete(row)" class="row-action row-action-danger">
  ...
  switch (state) {
    case 'active':
      return {
        ...
        dotClass: 'bg-emerald-500 shadow-[0_0_0_2px_rgb(0_199_190_/_0.18)]'
      }
    case 'blocked':
      return {
        label: t('admin.accounts.openai.compactUnsupported'),
        className: 'text-rose-600 dark:text-rose-300',
        dotClass: 'bg-rose-500 shadow-[0_0_0_2px_rgb(255_45_85_/_0.18)]'
      }
    case 'auto':
      return {
...
function getAntigravityTierClass(row: any): string {
  ...
  switch (tier) {
    ...
    case 'g1-pro-tier': return 'bg-blue-500/12 text-blue-600 dark:bg-blue-500/20 dark:text-blue-300'
    case 'g1-ultra-tier': return 'bg-purple-500/12 text-purple-600 dark:bg-purple-500/20 dark:text-purple-300'
    default: return ''
  }
}
...
.account-tools-menu-item {
  @apply flex w-full items-center gap-3 rounded-lg px-2.5 py-1.5 text-sm;
