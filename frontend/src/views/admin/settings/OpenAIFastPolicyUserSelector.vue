  ...
  <div ref="containerRef" class="relative">
    <div v-if="selectedUserIds.length > 0" class="mb-2 flex flex-wrap gap-2">
      ...
      >
        ...
        <span class="tabular shrink-0 text-gray-400">#{{ userId }}</span>
        <span
        ...
        <button
          type="button"
          class="shrink-0 rounded-md text-gray-400 transition-transform duration-instant ease-apple-out hover:text-red-600 active:scale-[0.96] dark:hover:text-red-400"
    ...
    <div
      ...
      class="dropdown mt-1 max-h-60 w-full origin-top overflow-auto"
    >
      <div v-if="searchLoading" class="px-3 py-2.5 text-sm text-gray-500 dark:text-gray-400">
        {{ t("common.loading") }}
      </div>
      <div
        v-else-if="availableResults.length === 0"
        class="px-3 py-2.5 text-sm text-gray-500 dark:text-gray-400"
      >
      ...
      <template v-else>
        <button
          ...
          :key="user.id"
          type="button"
          class="dropdown-item w-full justify-between gap-3 text-left"
          @click="selectUser(user)"
        >
          <!-- 悬停时 .dropdown-item 反白，子元素继承父色而不是钉死灰阶 -->
          <span class="min-w-0 truncate font-medium">
            {{ user.email }}
            <span v-if="user.deleted" class="ml-1 text-xs font-normal opacity-60">
              {{ t("admin.settings.openaiFastPolicy.userDeleted") }}
            </span>
          </span>
          <span class="tabular shrink-0 text-xs opacity-60">#{{ user.id }}</span>
        </button>
      </template>
    </div>
  </div>
