...
describe('admin AccountsView lite account list', () => {
  ...
  it('keeps the action menu open during internal scrolling but closes it on table scrolling', async () => {
    ...
    const trigger = wrapper.findAll('button').find(button => button.attributes('aria-label') === 'common.more')!
  ...
  it('loads the full account by id before opening edit, test, and stats actions', async () => {
    ...
    // v3 重设计：行内编辑为纯图标按钮，通过 aria-label 定位（i18n 在测试中返回原始 key）
    const editButton = wrapper.findAll('button').find(button => button.attributes('aria-label') === 'common.edit')
