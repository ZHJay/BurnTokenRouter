    ...
    <template #default>
      <div class="flex h-full min-h-0 flex-col">
        ...
        <div v-else class="flex min-h-0 flex-1 flex-col">
          <div v-if="items.length === 0" class="rounded-xl border border-dashed border-gray-200 p-10 text-center dark:border-dark-600">
          ...
          <div v-else class="table-container flex min-h-0 flex-1 flex-col overflow-hidden">
            <div class="min-h-0 flex-1 overflow-auto">
              <div v-if="!isDesktopViewport" class="divide-y" :style="{ borderColor: 'var(--separator)' }">
                <div v-for="(row, idx) in items" :key="idx" class="space-y-2 p-4">
                  <div class="flex flex-wrap items-center gap-2">
                    <span class="badge text-[10px]" :class="kindBadgeClass(row.kind)">
                    ...
                    <span class="tabular ml-auto text-[11px] text-gray-500 dark:text-gray-400">{{ formatDateTime(row.created_at) }}</span>
                  </div>
                  <div class="break-all text-xs text-gray-600 dark:text-gray-300">{{ row.model || '-' }}</div>
                  <div class="tabular flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-600 dark:text-gray-300">
                  ...
                  <div v-if="row.request_id" class="flex items-center gap-2">
                    ...
                    <button
                      class="btn btn-secondary btn-sm shrink-0 text-[10px]"
              ...
              <tbody>
                <tr v-for="(row, idx) in items" :key="idx">
                  <td class="tabular whitespace-nowrap px-4 py-3 text-xs text-gray-600 dark:text-gray-300">
                  ...
                  <td class="px-4 py-3">
                    <div v-if="row.request_id" class="flex items-center gap-2">
                      ...
                      <button
                        class="btn btn-secondary btn-sm text-[10px]"
                        @click="handleCopyRequestId(row.request_id)"
                      >
                        {{ t('admin.ops.requestDetails.copy') }}
                      </button>
