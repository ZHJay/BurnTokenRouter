...
const healthScoreColor = computed(() => {
  // 环形描边走 CSS 变量，深浅两套系统色由 style.css 统一切换
  if (isSystemIdle.value) return 'var(--label-tertiary)'
  const score = healthScoreValue.value
  if (score == null) return 'var(--label-tertiary)'
  if (score >= 90) return 'var(--sys-green)'
  if (score >= 60) return 'var(--sys-orange)'
  return 'var(--sys-red)'
})
...
<template>
  <!-- 头部是内容承载层：不透明高程，指标数字才不会被毛玻璃糊掉 -->
  <div :class="['card flex flex-col gap-4', props.fullscreen ? 'p-8' : 'p-6']">
    <!-- Top Toolbar -->
    <div class="flex flex-wrap items-center justify-between gap-4 pb-4 shadow-[inset_0_-0.5px_0_var(--separator)]">
      <div>
        ...
        <div v-if="!props.fullscreen" class="mt-1 flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
          <span class="flex items-center gap-1.5" :title="props.loading ? t('admin.ops.loadingText') : t('admin.ops.ready')">
            <span class="relative flex h-2 w-2">
              <span
                ...
                :style="{ background: props.loading ? 'var(--label-tertiary)' : 'var(--sys-green)' }"
              ></span>
            </span>
          ...
          <span>·</span>
          <span class="tabular">{{ t('common.refresh') }}: {{ props.lastUpdated ? props.lastUpdated.toLocaleString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' }).replace(/\//g, '-') : t('common.unknown') }}</span>

          <template v-if="props.autoRefreshEnabled && props.autoRefreshCountdown !== undefined">
            <span>·</span>
            <span class="tabular">{{ t('admin.ops.autoRefreshRemaining', { seconds: props.autoRefreshCountdown }) }}</span>
          </template>
        </div>
      </div>
      ...
      <div class="flex flex-wrap items-center gap-3">
        <template v-if="!props.fullscreen">
          ...
          <div class="mx-1 hidden h-4 w-[1px] sm:block" :style="{ background: 'var(--separator)' }"></div>
        ...
        <div v-if="!props.fullscreen" class="mx-1 hidden h-4 w-[1px] sm:block" :style="{ background: 'var(--separator)' }"></div>
    ...
    <div v-if="overview" class="grid grid-cols-1 gap-6 lg:grid-cols-12">
      ...
      <!-- 卡内嵌板：圆角比外层卡片小一级，嵌套关系才读得出来 -->
      <div :class="['card-inset lg:col-span-5', props.fullscreen ? 'p-6' : 'p-4']">
        <div class="grid h-full grid-cols-1 gap-6 md:grid-cols-[200px_1fr] md:items-center">
          <!-- 1) Health Score -->
          <div
            class="group relative flex cursor-pointer flex-col items-center justify-center rounded-lg py-2 transition-transform duration-instant ease-apple-out hover:bg-[var(--surface-hover)] active:scale-[0.98] md:pr-6 md:shadow-[inset_-0.5px_0_0_var(--separator)]"
          >
            ...
            >
              <!-- 悬浮诊断层：短暂出现的弹层用最轻材质 -->
              <div class="glass-thin glass-edge rounded-xl p-4">
                <h4 class="mb-3 flex items-center gap-2 pb-2 text-sm font-semibold text-gray-900 shadow-[inset_0_-0.5px_0_var(--separator)] dark:text-white">
                  <Icon name="brain" size="sm" :style="{ color: 'var(--accent)' }" />
                ...
                <div class="space-y-3">
                  <div v-for="(item, idx) in diagnosisReport" :key="idx" class="flex gap-3">
                    <div class="mt-0.5 shrink-0">
                      <svg v-if="item.type === 'critical'" class="h-4 w-4" :style="{ color: 'var(--sys-red)' }" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fill-rule="evenodd"
                      ...
                      <svg v-else-if="item.type === 'warning'" class="h-4 w-4" :style="{ color: 'var(--sys-orange)' }" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fill-rule="evenodd"
                      ...
                      <svg v-else class="h-4 w-4" :style="{ color: 'var(--accent)' }" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fill-rule="evenodd"
                    ...
                    <div class="flex-1">
                      ...
                      <div v-if="item.action" class="mt-1 flex items-center gap-1 text-[11px]" :style="{ color: 'var(--accent)' }">
                ...
                <div class="mt-3 pt-2 text-[10px] text-gray-400 shadow-[inset_0_0.5px_0_var(--separator)]">
            ...
            <div class="relative flex items-center justify-center">
              ...
              <div class="absolute flex flex-col items-center">
                <span :class="[props.fullscreen ? 'text-5xl' : 'text-3xl', 'tabular font-semibold tracking-[-0.026em]', healthScoreClass]">
                  {{ isSystemIdle ? t('admin.ops.idleStatus') : (overview.health_score ?? '--') }}
                </span>
                <span :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-semibold uppercase tracking-wider text-gray-400']">{{ t('admin.ops.health') }}</span>
              </div>
            </div>
            ...
            <div class="mt-4 text-center" v-if="!props.fullscreen">
              ...
              <div class="mt-1 text-xs font-semibold" :class="healthScoreClass">
                {{
                  isSystemIdle
          ...
          <div class="flex h-full flex-col justify-center py-2">
            ...
            <div :class="props.fullscreen ? 'space-y-4' : 'space-y-3'">
              <!-- Row 1: Current -->
              <div>
                <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-semibold uppercase text-gray-400']">{{ t('admin.ops.current') }}</div>
                <div class="mt-1 flex flex-wrap items-baseline gap-x-4 gap-y-2">
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'tabular font-semibold tracking-[-0.026em] text-gray-900 dark:text-white']">{{ displayRealTimeQps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-semibold text-gray-500']">QPS</span>
                  </div>
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'tabular font-semibold tracking-[-0.026em] text-gray-900 dark:text-white']">{{ displayRealTimeTps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-semibold text-gray-500']">{{ t('admin.ops.tps') }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <!-- Peak -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-semibold uppercase text-gray-400']">{{ t('admin.ops.peak') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-gray-600 dark:text-gray-400']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="tabular font-semibold text-gray-900 dark:text-white">{{ realtimeQpsPeakLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="tabular font-semibold text-gray-900 dark:text-white">{{ realtimeTpsPeakLabel }}</span>
                ...
                <!-- Average -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-semibold uppercase text-gray-400']">{{ t('admin.ops.average') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-gray-600 dark:text-gray-400']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="tabular font-semibold text-gray-900 dark:text-white">{{ realtimeQpsAvgLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="tabular font-semibold text-gray-900 dark:text-white">{{ realtimeTpsAvgLabel }}</span>
                      <span class="text-xs">{{ t('admin.ops.tps') }}</span>
                    </div>
                  </div>
                </div>
      ...
      <div class="grid h-full grid-cols-1 content-center gap-4 sm:grid-cols-2 lg:col-span-7 lg:grid-cols-3">
        <!-- Card 1: Requests -->
        <div class="card-inset rounded-xl p-4" style="order: 1;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-semibold uppercase text-gray-400">{{ t('admin.ops.requestsTitle') }}</span>
          ...
          <div class="mt-2 space-y-2 text-xs">
            <div class="flex justify-between">
              ...
              <span class="tabular font-semibold text-gray-900 dark:text-white">{{ totalRequestsLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-500">{{ t('admin.ops.tokens') }}:</span>
              <span class="tabular font-semibold text-gray-900 dark:text-white">{{ totalTokensLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-500">{{ t('admin.ops.avgQps') }}:</span>
              <span class="tabular font-semibold text-gray-900 dark:text-white">{{ qpsAvgLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-500">{{ t('admin.ops.avgTps') }}:</span>
              <span class="tabular font-semibold text-gray-900 dark:text-white">{{ tpsAvgLabel }}</span>
            </div>
          </div>
        </div>

        <!-- Card 2: SLA -->
        <div class="card-inset rounded-xl p-4" style="order: 2;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              <span class="text-[10px] font-semibold uppercase text-gray-400">{{ t('admin.ops.sla') }}</span>
          ...
          <div class="mt-2 tabular text-3xl font-semibold tracking-[-0.026em]" :class="getThresholdColorClass(getSLAThresholdLevel(slaPercent))">
            {{ slaPercent == null ? '-' : `${slaPercent.toFixed(3)}%` }}
          </div>
          <div class="progress mt-3 h-2 w-full">
            <div
              class="h-full rounded-full transition-[width] duration-base ease-spring"
          ...
          <div class="mt-3 text-xs">
            <div class="flex justify-between">
              ...
              <span class="tabular font-semibold text-red-600 dark:text-red-400">{{ formatNumber((overview.request_count_sla ?? 0) - (overview.success_count ?? 0)) }}</span>
            </div>
          </div>
        </div>

        <!-- Card 4: Request Duration -->
        <div class="card-inset rounded-xl p-4" style="order: 4;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-semibold uppercase text-gray-400">{{ t('admin.ops.latencyDuration') }}</span>
          ...
          <div class="mt-2 flex items-baseline gap-2">
            <div class="tabular text-3xl font-semibold tracking-[-0.026em] text-gray-900 dark:text-white">
              {{ durationP99Ms ?? '-' }}
            </div>
            <span class="text-xs font-semibold text-gray-400">ms (P99)</span>
          </div>
          <div class="mt-3 grid grid-cols-1 gap-x-3 gap-y-1 text-xs 2xl:grid-cols-2">
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">P95:</span>
              <span class="tabular font-semibold text-gray-900 dark:text-white">{{ durationP95Ms ?? '-' }}</span>
              <span class="text-gray-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">P90:</span>
              <span class="tabular font-semibold text-gray-900 dark:text-white">{{ durationP90Ms ?? '-' }}</span>
              <span class="text-gray-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">P50:</span>
              <span class="tabular font-semibold text-gray-900 dark:text-white">{{ durationP50Ms ?? '-' }}</span>
              <span class="text-gray-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">Avg:</span>
              <span class="tabular font-semibold text-gray-900 dark:text-white">{{ durationAvgMs ?? '-' }}</span>
              <span class="text-gray-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">Max:</span>
              <span class="tabular font-semibold text-gray-900 dark:text-white">{{ durationMaxMs ?? '-' }}</span>
              <span class="text-gray-400">ms</span>
            </div>
          </div>
        </div>
        ...
        <!-- Card 5: TTFT -->
        <div class="card-inset rounded-xl p-4" style="order: 5;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-semibold uppercase text-gray-400">TTFT</span>
          ...
          <div class="mt-2 flex items-baseline gap-2">
            <div class="tabular text-3xl font-semibold tracking-[-0.026em]" :class="getThresholdColorClass(getTTFTThresholdLevel(ttftP99Ms))">
              {{ ttftP99Ms ?? '-' }}
            </div>
            <span class="text-xs font-semibold text-gray-400">ms (P99)</span>
          </div>
          <div class="mt-3 grid grid-cols-1 gap-x-3 gap-y-1 text-xs 2xl:grid-cols-2">
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">P95:</span>
              <span class="tabular font-semibold" :class="getThresholdColorClass(getTTFTThresholdLevel(ttftP95Ms))">{{ ttftP95Ms ?? '-' }}</span>
              <span class="text-gray-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">P90:</span>
              <span class="tabular font-semibold" :class="getThresholdColorClass(getTTFTThresholdLevel(ttftP90Ms))">{{ ttftP90Ms ?? '-' }}</span>
              <span class="text-gray-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">P50:</span>
              <span class="tabular font-semibold" :class="getThresholdColorClass(getTTFTThresholdLevel(ttftP50Ms))">{{ ttftP50Ms ?? '-' }}</span>
              <span class="text-gray-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">Avg:</span>
              <span class="tabular font-semibold" :class="getThresholdColorClass(getTTFTThresholdLevel(ttftAvgMs))">{{ ttftAvgMs ?? '-' }}</span>
              <span class="text-gray-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-gray-500">Max:</span>
              <span class="tabular font-semibold" :class="getThresholdColorClass(getTTFTThresholdLevel(ttftMaxMs))">{{ ttftMaxMs ?? '-' }}</span>
              <span class="text-gray-400">ms</span>
            </div>
          </div>
        </div>
        ...
        <div class="card-inset rounded-xl p-4" style="order: 3;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-semibold uppercase text-gray-400">{{ t('admin.ops.requestErrors') }}</span>
              <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.errors')" />
            </div>
            <button v-if="!props.fullscreen" class="text-[10px] font-semibold text-blue-500 hover:underline" type="button" @click="openErrorDetails('request')">
              {{ t('admin.ops.requestDetails.details') }}
            </button>
          </div>
          <div class="mt-2 tabular text-3xl font-semibold tracking-[-0.026em]" :class="getThresholdColorClass(getRequestErrorRateThresholdLevel(errorRatePercent))">
            {{ errorRatePercent == null ? '-' : `${errorRatePercent.toFixed(2)}%` }}
          </div>
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-gray-500">{{ t('admin.ops.errorCount') }}:</span>
              <span class="tabular font-semibold text-gray-900 dark:text-white">{{ formatNumber(overview.error_count_sla ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-500">{{ t('admin.ops.businessLimited') }}:</span>
              <span class="tabular font-semibold text-gray-900 dark:text-white">{{ formatNumber(overview.business_limited_count ?? 0) }}</span>
            </div>
          </div>
        </div>

        <!-- Card 6: Upstream Errors -->
        <div class="card-inset rounded-xl p-4" style="order: 6;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-semibold uppercase text-gray-400">{{ t('admin.ops.upstreamErrors') }}</span>
              <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.upstreamErrors')" />
            </div>
            <button v-if="!props.fullscreen" class="text-[10px] font-semibold text-blue-500 hover:underline" type="button" @click="openErrorDetails('upstream')">
              {{ t('admin.ops.requestDetails.details') }}
            </button>
          </div>
          <div class="mt-2 tabular text-3xl font-semibold tracking-[-0.026em]" :class="getThresholdColorClass(getUpstreamErrorRateThresholdLevel(upstreamErrorRatePercent))">
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              ...
              <span class="tabular font-semibold text-gray-900 dark:text-white">{{ formatNumber(overview.upstream_error_count_excl_429_529 ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-gray-500">429/529:</span>
              <span class="tabular font-semibold text-gray-900 dark:text-white">{{ formatNumber((overview.upstream_429_count ?? 0) + (overview.upstream_529_count ?? 0)) }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="overview" class="mt-2 pt-4 shadow-[inset_0_0.5px_0_var(--separator)]">
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <!-- CPU -->
        <div class="card-inset p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-semibold uppercase tracking-wider text-gray-400">CPU</div>
            <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.cpu')" />
          </div>
          <div class="mt-1 tabular text-lg font-semibold tracking-[-0.02em]" :class="cpuPercentClass">
            {{ cpuPercentValue == null ? '-' : `${cpuPercentValue.toFixed(1)}%` }}
          </div>
          <div v-if="!props.fullscreen" class="mt-1 tabular text-[10px] text-gray-500 dark:text-gray-400">
        ...
        <div class="card-inset p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-semibold uppercase tracking-wider text-gray-400">{{ t('admin.ops.memory') }}</div>
            <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.memory')" />
          </div>
          <div class="mt-1 tabular text-lg font-semibold tracking-[-0.02em]" :class="memPercentClass">
        ...
        <div class="card-inset p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-semibold uppercase tracking-wider text-gray-400">{{ t('admin.ops.db') }}</div>
            <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.db')" />
          </div>
          <div class="mt-1 tabular text-lg font-semibold tracking-[-0.02em]" :class="dbMiddleClass">
            {{ dbMiddleLabel }}
          </div>
          <div v-if="!props.fullscreen" class="mt-1 tabular text-[10px] text-gray-500 dark:text-gray-400">
        ...
        <div class="card-inset p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-semibold uppercase tracking-wider text-gray-400">Redis</div>
            <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.redis')" />
          </div>
          <div class="mt-1 tabular text-lg font-semibold tracking-[-0.02em]" :class="redisMiddleClass">
            {{ redisMiddleLabel }}
          </div>
          <div v-if="!props.fullscreen" class="mt-1 tabular text-[10px] text-gray-500 dark:text-gray-400">
        ...
        <div class="card-inset p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-semibold uppercase tracking-wider text-gray-400">{{ t('admin.ops.goroutines') }}</div>
            <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.goroutines')" />
          </div>
          <div class="mt-1 tabular text-lg font-semibold tracking-[-0.02em]" :class="goroutineStatusClass">
        ...
        <div class="card-inset p-3">
          <div class="flex items-center justify-between gap-2">
            <div class="flex items-center gap-1">
              <div class="text-[10px] font-semibold uppercase tracking-wider text-gray-400">{{ t('admin.ops.jobs') }}</div>
              <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.jobs')" />
            </div>
            <button v-if="!props.fullscreen" class="text-[10px] font-semibold text-blue-500 hover:underline" type="button" @click="openJobsDetails">
              {{ t('admin.ops.requestDetails.details') }}
            </button>
          </div>

          <div class="mt-1 tabular text-lg font-semibold tracking-[-0.02em]" :class="jobsStatusClass">
            {{ jobsStatusLabel }}
          </div>
        ...
        >
          <div class="flex items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
              <span v-if="hb.last_duration_ms != null" class="tabular font-mono">{{ hb.last_duration_ms }}ms</span>
              <span class="tabular">{{ formatTimeShort(hb.updated_at) }}</span>
            </div>
          </div>

          <div class="mt-2 grid grid-cols-1 gap-2 text-xs text-gray-600 dark:text-gray-300 sm:grid-cols-2">
            <div>
              {{ t('admin.ops.lastSuccess') }} <span class="tabular font-mono">{{ formatTimeShort(hb.last_success_at) }}</span>
            </div>
            <div>
              {{ t('admin.ops.lastError') }} <span class="tabular font-mono">{{ formatTimeShort(hb.last_error_at) }}</span>
            </div>
            <div>
