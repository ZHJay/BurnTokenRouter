...
<template>
  <section class="card p-4 md:p-5">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      <h3 class="text-sm font-semibold tracking-[-0.01em] text-gray-900 dark:text-white">
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <template v-else>
          ...
          <span class="tabular text-xs text-gray-500 dark:text-gray-400">
    ...
    <div
      v-if="errorMessage"
      class="mb-4 rounded-lg px-3 py-2 text-xs"
    ...
    <div v-else class="space-y-3">
      <div class="table-container overflow-hidden">
        <div class="max-h-[420px] overflow-auto">
          <div v-if="!isDesktopViewport" class="divide-y" :style="{ borderColor: 'var(--separator)' }">
            <div v-for="row in items" :key="row.model" class="space-y-2 p-3">
              ...
              <div class="grid grid-cols-2 gap-x-3 gap-y-1 text-xs">
                <div class="flex items-baseline justify-between gap-2">
                  ...
                  <span class="tabular text-gray-700 dark:text-gray-200">{{ formatInt(row.request_count) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-gray-500 dark:text-gray-400">{{ t('admin.ops.openaiTokenStats.table.avgTokensPerSec') }}</span>
                  <span class="tabular text-gray-700 dark:text-gray-200">{{ formatRate(row.avg_tokens_per_sec) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-gray-500 dark:text-gray-400">{{ t('admin.ops.openaiTokenStats.table.avgFirstTokenMs') }}</span>
                  <span class="tabular text-gray-700 dark:text-gray-200">{{ formatRate(row.avg_first_token_ms) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-gray-500 dark:text-gray-400">{{ t('admin.ops.openaiTokenStats.table.totalOutputTokens') }}</span>
                  <span class="tabular text-gray-700 dark:text-gray-200">{{ formatInt(row.total_output_tokens) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-gray-500 dark:text-gray-400">{{ t('admin.ops.openaiTokenStats.table.avgDurationMs') }}</span>
                  <span class="tabular text-gray-700 dark:text-gray-200">{{ formatInt(row.avg_duration_ms) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-gray-500 dark:text-gray-400">{{ t('admin.ops.openaiTokenStats.table.requestsWithFirstToken') }}</span>
                  <span class="tabular text-gray-700 dark:text-gray-200">{{ formatInt(row.requests_with_first_token) }}</span>
                </div>
              </div>
            </div>
          </div>
          <table v-else class="table min-w-full text-left text-xs md:text-sm">
            ...
            <thead class="sticky top-0 z-10 [&_th]:bg-[var(--surface)] [&_th]:backdrop-blur-none">
              <tr>
                ...
                <th class="px-2 py-2 normal-case">{{ t('admin.ops.openaiTokenStats.table.avgTokensPerSec') }}</th>
                <th class="px-2 py-2 normal-case">{{ t('admin.ops.openaiTokenStats.table.avgFirstTokenMs') }}</th>
                <th class="px-2 py-2 normal-case">{{ t('admin.ops.openaiTokenStats.table.totalOutputTokens') }}</th>
                <th class="px-2 py-2 normal-case">{{ t('admin.ops.openaiTokenStats.table.avgDurationMs') }}</th>
                <th class="px-2 py-2 normal-case">{{ t('admin.ops.openaiTokenStats.table.requestsWithFirstToken') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr
              ...
              >
                ...
                <td class="tabular px-2 py-2">{{ formatInt(row.request_count) }}</td>
                <td class="tabular px-2 py-2">{{ formatRate(row.avg_tokens_per_sec) }}</td>
                <td class="tabular px-2 py-2">{{ formatRate(row.avg_first_token_ms) }}</td>
                <td class="tabular px-2 py-2">{{ formatInt(row.total_output_tokens) }}</td>
                <td class="tabular px-2 py-2">{{ formatInt(row.avg_duration_ms) }}</td>
                <td class="tabular px-2 py-2">{{ formatInt(row.requests_with_first_token) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
      </div>
      <div v-if="viewMode === 'topn'" class="tabular mt-3 text-xs text-gray-500 dark:text-gray-400">
