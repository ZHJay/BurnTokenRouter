...
func TestFrontendServer_Middleware(t *testing.T) {
	...
	t.Run("serves_static_files", func(t *testing.T) {
		...
		// Request for an existing, non-fingerprinted static file. logo.svg is the
		// only top-level asset frontend/public ships, so it exercises the
		// "real embedded file, no immutable caching" branch.
		w := httptest.NewRecorder()
